...
function RegisterForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Pre-check: tell the user clearly if the email is already registered
      // so we don't depend on opaque error messages from the auth provider.
      ...
      const isDuplicate =
        lower.includes('already') || lower.includes('exists') || lower.includes('invalidaccountid');
      ...
      toast({
        variant: 'destructive',
        title: isDuplicate ? 'Email already registered' : 'Sign up failed',
        description: message,
      });
    } finally {
      setIsLoading(false);
