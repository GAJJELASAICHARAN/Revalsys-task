    ...
    <div className="container mx-auto px-4 py-6">
      ...
      <div className="mb-5">
        <h1 className="text-xl font-normal text-[#0f1111]">
          {results.length > 0 ? (
            <>
              <span className="text-[#565959] text-base">
          ...
          <div className="mt-3 flex items-start gap-2 p-3 rounded bg-[#fffbe6] border border-[#FFD814]">
            ...
            <p className="text-sm text-[#0f1111]">
              ...
              {suggestions.category && (
                <>
                  Category: <em>{suggestions.category}</em>
                </>
              )}
              {suggestions.category && suggestions.priceRange && ' · '}
              {suggestions.priceRange && (
                <>
                  Budget: ₹{suggestions.priceRange.min}–₹{suggestions.priceRange.max}
                </>
              )}
            </p>
          </div>
      ...
      <div className="mt-12 border-t border-[#e7e7e7] pt-8">
        ...
        <div className="flex flex-wrap gap-2">
          {['Gaming laptop', 'Budget smartphone', 'Premium tablet', 'Wireless earbuds', 'Smartwatch', 'Ultrabook'].map(
            s => (
              <Link
                key={s}
                href={`/search?q=${encodeURIComponent(s)}`}
